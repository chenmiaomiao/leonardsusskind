...
\qquad
\mathcal H(x)=\Psi^\dagger(x)
\left(-\frac{\nabla^2}{2m}+V(x)\right)\Psi(x).
\label{eq:l07-hamiltonian-density}
\end{equation}
The integrand is a natural local energy density for this Hamiltonian. Its local form is not unique: integration by parts, for example, replaces the kinetic term by
\begin{equation}
\mathcal H_{\rm kin}(x)
