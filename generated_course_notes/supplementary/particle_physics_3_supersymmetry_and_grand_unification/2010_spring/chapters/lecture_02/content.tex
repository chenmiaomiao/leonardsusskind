...
\end{equation}
One traversal is nontrivial, while two traversals compose to the trivial class. Equivalently, \(SU(2)\) is the double cover of \(SO(3)\).\editorialnote{The \(SO(3)\), \(SU(2)\), and fundamental-group language makes the topology explicit; the lecture establishes the same fact through the arm and string demonstrations.} This is the geometric prehistory of the fermion sign.
...
\begin{equation}
\hbox{Higgs protection}
\ \Longrightarrow\
\hbox{effective boson--fermion cancellation}
\ \Longrightarrow\
\Delta m_{\mathrm{SUSY}}\ \hbox{not too large}.
\end{equation}
...
+
\mathcal A_{\mathrm{loop}}^{(\widetilde e)}
=0
\quad\hbox{only when the full couplings match.}
\end{equation}
...
\begin{tabular}{lll}
\toprule
ordinary state & superpartner & typical spin change\\
\midrule
scalar Higgs, \(j=0\) & Higgsino, \(j=\tfrac12\) & \(+\tfrac12\)\\
gauge boson, \(j=1\) & gaugino, \(j=\tfrac12\) & \(-\tfrac12\)\\
matter fermion, \(j=\tfrac12\) & sfermion, \(j=0\) & \(-\tfrac12\)\\
graviton, \(j=2\) & gravitino, \(j=\tfrac32\) & \(-\tfrac12\)\\
\bottomrule
\end{tabular}
\end{center}
