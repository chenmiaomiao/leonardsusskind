...
\end{equation}
Different values of \(\phi\) correspond to different amounts of energy stored per unit physical volume.
...
\end{equation}
For an expanding universe \(H>0\), the second term always opposes the field velocity. It therefore behaves like viscosity. A rock released in thick honey initially accelerates, but drag grows with speed until the rock reaches terminal velocity. The mnemonic \emph{\(H\) for honey} captures the analogy, while the equation supplies its actual content. If \(H\) changes with cosmic time, the effective viscosity changes too, much as ordinary viscosity changes with temperature.
...
\end{equation}
This is inflation. A rough early-universe doubling time near \(10^{-32}\,\mathrm{s}\) conveys its violence; the number is illustrative and depends on the inflationary energy scale.
...
\label{eq:c09-efolds}
\end{equation}
We shall use \(N\gtrsim60\) as the useful order-of-magnitude lower bound. The exact requirement depends on the inflationary scale and reheating history; estimates in the broad neighborhood of \(40\)--\(80\) occur in different models. More initial roughness or unwanted relic density can demand more inflation.
...
\section{What the Argument Has Established}
