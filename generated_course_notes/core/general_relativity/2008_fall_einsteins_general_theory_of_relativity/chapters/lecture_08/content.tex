...
\begin{figure}[tbp]
  ...
  \caption{The infinitesimal loop and the values
  \(V_A,V_B,V_C,V_D,V_A'\) of the transported vector. The dashed
  mismatch at \(A\) represents a change of orientation, not a failure of
  the path to close.}
...
\end{align}
The first antisymmetry follows from metric-compatible parallel transport,
which preserves inner products. The second follows from the commutator.
The third exchanges the two antisymmetric index pairs. The last is the
algebraic, or first, Bianchi identity.
