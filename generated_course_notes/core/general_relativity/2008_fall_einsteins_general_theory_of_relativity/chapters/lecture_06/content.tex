...
products. Write \(u\mathbin{\cdot}v\equiv g_{mn}u^m v^n\). If \(u^m\)
and \(v^m\) are both parallel transported, then
\begin{align}
  \frac{D}{Ds}\bigl(u\mathbin{\cdot}v\bigr)
  &=\nonumber\\[-1.1ex]
  &\quad
...
\begin{equation}
  \begin{gathered}
    \text{energy and momentum}\\[-0.25ex]
    \downarrow\\[-0.25ex]
    \text{space-time curvature}\\[-0.25ex]
    \downarrow\\[-0.25ex]
    \text{geodesic motion}
  \end{gathered}
